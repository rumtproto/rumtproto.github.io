...
##### 8) Thereafter, auth\_key equals `pow(g, {ab}) mod dh_prime`; on the server, it is computed as `pow(g_b, a) mod dh_prime`, and on the client as `(g_a)^b mod dh_prime`.
...
An example of a complete list of queries required to generate an authorization key is shown on [a separate page](/mtproto/samples-auth_key/).
