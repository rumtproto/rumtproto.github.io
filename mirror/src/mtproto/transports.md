...
section: mtproto
description: "Enables the delivery of encrypted containers together with the external header (hereinafter, _Payload_) from client to server and back. Multiple transport protocols are defined:"
crumbs: [{"title":"Mobile Protocol","url":"/mtproto/"},{"title":"Transport protocols","url":"/mtproto/transports/"}]
layout: layout.njk
---
...
The TCP transport is implemented simply by sending the payloads generated by the chosen [MTProto transport](/mtproto/mtproto-transports/) over a plain TCP socket on ports 80, 443, 5222 or other (a different port number may be returned by [help.getConfig](/method/help.getConfig/)).  
Note that if the `this_port_only` [dcOption](/constructor/dcOption/) flag is set, clients must use only the specified port, without trying any other port.  
Also note that if the `force_try_ipv6` [config](/constructor/config/) flag is set, clients must prioritize IPv6 over IPv4 for all MTProto transports, even if an IPv4 connection is available.

Framing is managed by the chosen [MTProto transport protocol](/mtproto/mtproto-transports/).
...
Note: when implementing browser clients, [WebSocket transport](#websocket) is recommended instead of HTTP, thanks to its full-duplex stream logic similar to TCP's; this removes the need for [HTTP long polling](/mtproto/service_messages/#http-wait-long-poll) and eventual delays while relaying RPC replies.

Implemented over HTTP/1.1 (with keepalive) running over the traditional TCP Port 80. [HTTPS can also be used](#https).

Message framing is **not** managed by [MTProto transport protocols](/mtproto/mtproto-transports/); it is instead handled by the HTTP protocol itself. Transport errors are also **not** transmitted [the usual way](/mtproto/mtproto-transports/#transport-errors), instead they are simply returned as normal HTTP status codes.
