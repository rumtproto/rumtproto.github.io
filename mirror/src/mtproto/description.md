...
section: mtproto
description: "This article describes the basic layer of the MTProto protocol version 2.0 (Cloud chats, server-client encryption). The principal differences from version 1.0 (described here for…"
crumbs: [{"title":"Mobile Protocol","url":"/mtproto/"},{"title":"Mobile Protocol: Detailed Description","url":"/mtproto/description/"}]
layout: layout.njk
---
...
This article describes the basic layer of the MTProto protocol version 2.0 (Cloud chats, server-client encryption). The principal differences from version 1.0 ([described here](/mtproto/description_v1/) for reference) are as follows:
...
#### Message Sequence Number (msg\_seqno)
...
A client **must always** mark all API-level RPC queries as content-related, or else a [bad\_msg\_notification](/mtproto/service_messages_about_messages/#notice-of-ignored-error-message) with `error_code=35` will be emitted.

A client **must never** mark `msgs_ack`, `msg_container`, `msg_copy`, `gzip_packed` constructors (i.e. [containers](/mtproto/service_messages/#containers) and acknowledgements) as content-related, or else a [bad\_msg\_notification](/mtproto/service_messages_about_messages/#notice-of-ignored-error-message) with `error_code=34` will be emitted.
...
_For the obsolete MTProto 1.0, msg\_key, aes\_key, and aes\_iv were computed differently (see [this document](/mtproto/description_v1/#defining-aes-key-and-initialization-vector) for reference)._
...
#### Encrypted Message: _encrypted\_data_
