...
The `id` field of [peers »](/api/peers/) (i.e. users, chats, channels, secret chats) contains four distinct sequences of unique 64-bit IDs used to identify a specific user, chat, channel or secret chat.
...
It's a good idea to transform peer IDs to bot dialog API IDs even if you do decide to use separate databases to [store info about peers](/api/peers/), as it will make IDs more visually recognizable both for you and your users, as well as guarantee compatibility with the bot API, and allow your client to easily identify the type of a peer just by using its ID, thanks to the range checks specified below.

Example implementation: [tdlib (bot API)](https://github.com/tdlib/td/blob/c19deb1c9fb6d1ecd37db8f4d8148666bbdbb79d/td/telegram/DialogId.cpp), [MadelineProto](https://github.com/danog/tg-dialog-id).

### User IDs

[User](/constructor/user/) IDs in the MTProto API range from `1` to `0xffffffffff` (inclusive).
...
[Chat](/constructor/chat/) IDs in the MTProto API range from `1` to `999999999999` (inclusive).
...
[Supergroup/channel](/constructor/channel/) IDs share the same sequence in the MTProto API, and they range from `1` to `997852516352` (inclusive).
...
**Note**: the above range does **not** apply to [monoforums »](/api/monoforum/), which are a special kind of forum used for direct channel messages, represented by [channel](/constructor/channel/) constructors with the `monoforum` flag set: see [here »](#monoforum-ids) for the range used by monoforums.

### Monoforum IDs

[Monoforums »](/api/monoforum/) are a special kind of forum used for direct channel messages, represented by [channel](/constructor/channel/) constructors with the `monoforum` flag set.

The MTProto IDs of monoforums range from `1002147483649` to `3000000000000`.

To convert MTProto monoforum IDs to bot API monoforum IDs, add `1000000000000` and make them negative (and vice versa): this procedure is the same used for [supergroup/channel IDs](#supergroup-channel-ids), the only change is the source MTProto range.
...
[Secret chat IDs](/api/end-to-end/) in the MTProto API range from `-2147483648` to `2147483647` (inclusive, treat the secret chat ID as a signed little-endian 32-bit integer).
