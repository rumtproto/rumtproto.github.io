...
How to work with geolocation-based features like geogroups and the nearby users feature.
...
Use [contacts.getLocated](/method/contacts.getLocated/) to fetch a list of nearby users and groups.
...
Pass a `geo_point` to [channels.createChannel](/method/channels.createChannel/) when creating a [supergroup](/api/channel/) in order to create a geogroup associated to a geolocation, that will be returned to [nearby users »](#fetching-nearby-users-and-geogroups).

A textual description of the location (1-64 UTF-8 chars) should also be passed in `address`.

Use [channels.editLocation](/method/channels.editLocation/) to change the group's location.
...
Our current location may be [advertised to other users](#fetching-nearby-users-and-geogroups) using [contacts.getLocated](/method/contacts.getLocated/): in this case the `self_expires` flag **must always be set**.

Note that if the current user is already advertising their location using the [Telegram Business location feature »](/api/business/#location) (even without a `geo_point`, just with a textual `address`), the method will return a `BUSINESS_ADDRESS_ACTIVE` error, indicating that the location may only be changed (or removed) using [account.updateBusinessLocation »](/method/account.updateBusinessLocation/), instead of [contacts.getLocated](/method/contacts.getLocated/).
