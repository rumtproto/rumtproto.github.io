...
section: api
description: "Versioning in the API is supported by so-called TL layers."
crumbs: [{"title":"API","url":"/api/"},{"title":"Calling API Methods","url":"/api/invoking/"}]
layout: layout.njk
---
...
The helper method **[invokeWithLayer](/method/invokeWithLayer/)** can be used only together with [initConnection](/method/initConnection/): the present layer will be saved with all other parameters of the client and any future requests will be using this saved value. [See more below](#saving-client-info).
...
Clients should treat this as a `500` server error, and handle it by closing and reopening the TCP socket, re-initializing the session with [initConnection](/method/initConnection/) and invoking [getDifference](/api/updates/).

#### [List of Available Layers](/api/layers/)
...
The helper method **[initConnection](/method/initConnection/)** accepts client parameters. This method must be called when first calling the API after the application has restarted or in case the value of one of the parameters could have changed.

**initConnection** must also be called after each [auth.bindTempAuthKey](/method/auth.bindTempAuthKey/).

When calling this method, the current layer used by the client is also saved (the layer in which [initConnection](/method/initConnection/) was wrapped is used). After a successful call to [initConnection](/method/initConnection/) it is no longer necessary to wrap each API call in **invokeWithLayerN**.
...
[invokeWithoutUpdates](/method/invokeWithoutUpdates/) can be used to invoke a request without subscribing the used connection for [updates](/api/updates/) (this is enabled by default for [file queries](/api/files/)).
...
-   OR Wait for receipt of an RPC error for any of the queries mentioned in `msg_ids`, and then send the following new query:
...
The schema and constructor information are given in the [protocol documentation](/mtproto/service_messages/#packed-object).

#### Data Compression when Making a Request

Before transmitting a query, the string containing the entire body of the serialized high-level query (starting with the method number) must be compressed using gzip. If the resulting string is smaller than the original, it makes sense to transmit the [gzip\_packed](/mtproto/service_messages/#packed-object) constructor.
...
By default, the server can compress the response to any request as well as [updates](/api/updates/), in accordance with the rules stated above. If the [gzip\_packed](/mtproto/service_messages/#packed-object) constructor is received as a response in rpc\_result, then the string that follows must be extracted and uncompressed. Processing then continues on the resulting new string.
