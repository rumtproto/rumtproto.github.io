...
crumbs: [{"title":"API","url":"/api/"},{"title":"Telegram Business","url":"/api/business/"}]
layout: layout.njk
---
...
All Telegram Business features are available for free to [Premium »](/api/premium/) subscribers.

Additionally, [connected bots »](/api/bots/connected-business-bots/) are also available to non-Premium users.
...
Businesses can display their hours of operation; this info will be contained in [userFull](/constructor/userFull/).`business_work_hours`.  
To set this information use [account.updateBusinessWorkHours](/method/account.updateBusinessWorkHours/), passing a [businessWorkHours](/constructor/businessWorkHours/) constructor with:

-   `weekly_open` - A list of time intervals (max 28) represented by [businessWeeklyOpen](/constructor/businessWeeklyOpen/), indicating the opening hours of their business.  
    The time intervals in [businessWeeklyOpen](/constructor/businessWeeklyOpen/).`start/end_minute` are specified in minutes of the week.  
...
-   `timezone_id` - An ID of one of the timezones returned by [help.getTimezonesList](/method/help.getTimezonesList/).  
...
-   `open_now` - Ignored when passed by the client, only populated by the server in [userFull](/constructor/userFull/).`business_work_hours`, indicating whether the business is currently open according to the current time and the values in `weekly_open` and `timezone`.

Changing the opening hours will emit an [updateUser](/constructor/updateUser/).

To remove all info about opening hours, invoke [account.updateBusinessWorkHours](/method/account.updateBusinessWorkHours/) without populating the `business_work_hours` flag.
...
Businesses may advertise their location using [account.updateBusinessLocation](/method/account.updateBusinessLocation/); this information is then returned to users in [userFull](/constructor/userFull/).`business_location`.
...
If a `geo_point` is set, the business' location will also be advertised to geographically close users using the nearby users feature, [as described here »](/api/nearby/#fetching-nearby-users-and-geogroups).

Note that even if just the `address` is set (with or without a `geo_point`), the current geolocation of the user may not be changed using [contacts.getLocated](/method/contacts.getLocated/) method (i.e. for the [nearby users feature](/api/nearby/#advertising-our-current-location)): it will return a `BUSINESS_ADDRESS_ACTIVE` error, indicating that the location may only be changed (or removed) using [account.updateBusinessLocation »](/method/account.updateBusinessLocation/).

Changing the location will emit an [updateUser](/constructor/updateUser/).
...
A quick reply shortcut has a name (shown to the user), a numeric ID and a set of associated [message](/constructor/message/)s.
...
To fetch basic info about all existing quick reply shortcuts, use [messages.getQuickReplies](/method/messages.getQuickReplies/).
...
Note that the `hash` is used for caching, and is generated **not** by using the message IDs, but rather the following set of integers, passed to the [usual hashing algorithm](/api/offsets/#hash-generation).
...
The method will return [messages.quickRepliesNotModified](/constructor/messages.quickRepliesNotModified/) if the passed hash is equal to the server-side hash, meaning that the order, names and last messages of the quick replies haven't changed.

Editing some intermediate message in a quick reply will not change the hash, which is why [messages.getQuickReplyMessages](/method/messages.getQuickReplyMessages/) should be used to fetch (a subset or all) messages in a quick reply shortcut, and its hash is generated as follows:
...
The generated list of integers must then be passed to the [usual hashing algorithm](/api/offsets/#hash-generation).
...
To add a message to a quick reply shortcut, use [messages.sendMessage](/method/messages.sendMessage/), [messages.sendMedia](/method/messages.sendMedia/), [messages.sendMultiMedia](/method/messages.sendMultiMedia/), [messages.sendInlineBotResult](/method/messages.sendInlineBotResult/) or [messages.forwardMessages](/method/messages.forwardMessages/) with `peer=`[inputPeerSelf](/constructor/inputPeerSelf/), populating the `quick_reply_shortcut` flag with an [InputQuickReplyShortcut](/type/InputQuickReplyShortcut/) constructor.

The [InputQuickReplyShortcut](/type/InputQuickReplyShortcut/) constructor can be either:

-   [inputQuickReplyShortcut](/constructor/inputQuickReplyShortcut/) to use the shortcut name (used to create a new shortcut)
-   [inputQuickReplyShortcutId](/constructor/inputQuickReplyShortcutId/) to use the shortcut ID (used to edit an existing shortcut)

If the specified quick reply does not exist, it will be created when invoking the above methods, emitting an additional [updateNewQuickReply](/constructor/updateNewQuickReply/) to all logged-in sessions.

The numeric ID of the (newly created or existing) shortcut will be contained in the `quick_reply_shortcut_id` field of the [message](/constructor/message/)s contained in the [updateQuickReplyMessage](/constructor/updateQuickReplyMessage/) updates returned by invoking the above methods.  
The returned messages will use a common message `id` sequence, unrelated to any other message ID sequence used in API.

A maximum of [appConfig.`quick_replies_limit`](/api/config/#quick-replies-limit) shortcuts may be created, otherwise a `400 QUICK_REPLIES_TOO_MUCH` error will be emitted by the methods above.

Each shortcut can contain a maximum of [appConfig.`quick_reply_messages_limit`](/api/config/#quick-reply-messages-limit) messages, otherwise a `400 REPLY_MESSAGES_TOO_MUCH` error will be emitted by the methods above.

Make sure to invoke [messages.checkQuickReplyShortcut](/method/messages.checkQuickReplyShortcut/) passing the shortcut name before offering the user the choice to add a message to a quick reply shortcut, to make sure that none of the limits specified above were reached.
...
To send a quick reply shortcut to a user, use [messages.sendQuickReplyMessages](/method/messages.sendQuickReplyMessages/), passing the shortcut ID.

Quick replies may only be sent by users, in private chats with other users. The UI used to select quick replies should be similar to the one used for [bot commands](/api/bots/commands/), using the shortcut name instead of the bot command name.
...
Use [messages.editMessage](/method/messages.editMessage/) to edit a message in a quick reply shortcut, passing the message ID in `id` and the shortcut ID in `quick_reply_shortcut_id`.

This will emit an [updateQuickReplyMessage](/constructor/updateQuickReplyMessage/) update.
...
To delete one or more messages from a quick reply shortcut, use [messages.deleteQuickReplyMessages](/method/messages.deleteQuickReplyMessages/), passing the shortcut ID, and the list of message IDs to delete.  
This will also emit an [updateDeleteQuickReplyMessages](/constructor/updateDeleteQuickReplyMessages/) update.
...
Use [messages.editQuickReplyShortcut](/method/messages.editQuickReplyShortcut/) to rename a shortcut.  
...
Use [messages.reorderQuickReplies](/method/messages.reorderQuickReplies/) to reorder quick reply shortcuts (not the messages within them); pass in `order` the IDs of all created quick reply shortcuts, in the desired order.  
This will emit an [updateQuickReplies](/constructor/updateQuickReplies/) update to other logged-in sessions.
...
To completely delete a quick reply shortcut, use [messages.deleteQuickReplyShortcut](/method/messages.deleteQuickReplyShortcut/).  
This will also emit an [updateDeleteQuickReply](/constructor/updateDeleteQuickReply/) update to other logged-in sessions (and _no_ [updateDeleteQuickReplyMessages](/constructor/updateDeleteQuickReplyMessages/) updates, even if all the messages in the shortcut are also deleted by this method).
...
Use [account.updateBusinessGreetingMessage](/method/account.updateBusinessGreetingMessage/) to set a (list of) greeting messages in `message`; to disable greetings, call the method without populating the `message` field.

If populated, the `message` field must contain a [inputBusinessGreetingMessage](/constructor/inputBusinessGreetingMessage/), see [the constructor page »](/constructor/inputBusinessGreetingMessage/) for a description of its contents and the length limits for the intro title and description.

The currently configured business greeting can be fetched by the current user through [userFull](/constructor/userFull/).`business_greeting_message`, represented by a [businessGreetingMessage constructor »](/constructor/businessGreetingMessage/), equivalent to the input counterpart.
...
Use [account.updateBusinessAwayMessage](/method/account.updateBusinessAwayMessage/) to set a (list of) away messages in `message`; to disable greetings, call the method without populating the `message` field.

If populated, the `message` field must contain a [inputBusinessAwayMessage](/constructor/inputBusinessAwayMessage/), see [the constructor page »](/constructor/inputBusinessAwayMessage/) for a description of its contents.

The currently configured business away message can be fetched by the current user through [userFull](/constructor/userFull/).`business_away_message`, represented by a [businessAwayMessage constructor »](/constructor/businessAwayMessage/), equivalent to the input counterpart.
...
Telegram Business allows to configure the message and sticker of the [profile introduction »](/api/profile/#introduction), shown to new users that don't have a private chat with us.

Use [account.updateBusinessIntro](/method/account.updateBusinessIntro/) to set a custom business introduction; invoke the same method without setting the `intro` flag to remove the custom business introduction, defaulting to a randomly-chosen introduction message and sticker (see [here »](/api/profile/) for more info on default profile introductions).

Changing the business introduction will emit an [updateUser](/constructor/updateUser/), and the business introduction will be contained in [userFull](/constructor/userFull/).`intro`.

Note that the greeting sticker selection UI should offer a list of categories to quickly filter results by a (list of) emojis, or by some other criteria, see [here »](/api/emoji-categories/) for more info.
...
[Business chat deep links](/api/links/#business-chat-links) allow business owners to share pre-made links to their Telegram business account, optionally filling out the message input field with a pre-prepared message (with support for [styled text entities »](/api/entities/)).

There is no additional limit (apart from the usual [message\_length\_max](/constructor/config/)) on the prepared message length (because the message text is _not_ present in the URL's query string, rather it's fetched from the server using the link slug); each created business chat deep link also has a view counter.

Use [account.createBusinessChatLink](/method/account.createBusinessChatLink/) to create a business chat link, specifying in [inputBusinessChatLink](/constructor/inputBusinessChatLink/) the message to pre-fill when users open the link, as well as a human-readable name for the link in `title`, useful when managing created links.  
The created [deep link](/api/links/#business-chat-links) is returned in the `link` field, and may be shared directly or as a QR code.

Use [account.getBusinessChatLinks](/method/account.getBusinessChatLinks/) to fetch info about all created business chat links (including the view counter, indicating the number of times a business chat link was resolved (clicked on, scanned) by a user).

Use [account.editBusinessChatLink](/method/account.editBusinessChatLink/) to edit the information of a business chat link.

Use [account.deleteBusinessChatLink](/method/account.deleteBusinessChatLink/) to delete a business chat link.

Use [account.resolveBusinessChatLink](/method/account.resolveBusinessChatLink/) to open a business chat link, obtaining the peer to contact and (if set) the message to pre-fill, also increasing the link's view counter.

The last three methods listed above take a business chat link slug, which should be extracted from a business chat link as specified [here »](/api/links/#business-chat-links).

All listed methods except [account.resolveBusinessChatLink](/method/account.resolveBusinessChatLink/) require a Telegram Business subscription (currently included in [Telegram Premium](/api/premium/) subscriptions).

An account may create a maximum of [business\_chat\_links\_limit](/api/config/#business-chat-links-limit) business chat links: attempts to exceed this limit will emit a `CHATLINKS_TOO_MUCH` RPC error, prompting the user to delete older links.

### Connected bots

Users can connect Telegram bots that will process and answer messages **on their behalf**. This allows them to **seamlessly integrate** any existing tools and workflows, or add **AI assistants** that manage their chats.

See [here »](/api/bots/connected-business-bots/) for full documentation on business bots connected to business user accounts.
...
Since Telegram Business is currently included with the [Telegram Premium](/api/premium/) subscription, and Telegram Premium subscribers do not see [sponsored messages in channels](/api/sponsored-messages/).
...
For this reason, the Telegram Business settings page should contain a toggle to re-enable Telegram ads for the current account, which should trigger a call to [account.toggleSponsoredMessages](/method/account.toggleSponsoredMessages/).

The current value of the toggle will be stored in [userFull](/constructor/userFull/).`sponsored_enabled`.

### Folder tags

Telegram Business users can use [folder tags, see here »](/api/folders/#folder-tags) for more info.
...
[help.getAppConfig](/method/help.getAppConfig/) to fetch info on how to build the Telegram Business modal, returning a list of Business feature identifiers in the [`business_promo_order` appConfig field](/api/config/#business-promo-order): the modal should contain a row for each returned feature, ordered as specified in the returned array.
...
Users can [connect business bots to their account »](/api/bots/connected-business-bots/)

#### `emoji_status`

Business users can set a [custom emoji status](/api/emoji-status/).

#### `folder_tags`

Business users can enable [folder tags](/api/folders/#folder-tags).

#### `stories`

Business users can use [stories](/api/stories/) to showcase products (i.e. no additional functionality needs to be implemented by clients, it's just another way to use stories).
