...
**Дата копии оригинального сайта / content snapshot date: `2026-08-28`** (3464 страницы / pages).
...
из `backup/latest/` и `site/`, второй — из `crawled/`. Для локальной разработки:
...
Проект хранит актуальную копию скачанных страниц оригинала в `backup/latest/`; дата снимка записана
в `manifest.json`. **Весь контент для сборки извлекается только из этого бэкапа — сборка не
обращается к сети**. Мета-описания и OG-теги страниц генерируются из текста оригинала (первый
абзац страницы), без добавления своего текста.
...
же датой **дополняет** `backup/latest/`: уже скачанные файлы не трогаются, качаются только
недостающие. Другая дата создаёт полный снимок во временном `backup/.latest-next/` и заменяет
`latest` только после успешного завершения; `--fresh` принудительно делает то же в пределах одного
дня. При дополнении страница, пропавшая из нового прогона, останавливает запись манифеста с ошибкой —
...
(ссылки, которые страницами зеркала быть не могут, с причиной). При дополнении оба списка
переиспользуются следующим прогоном, поэтому уже выясненный адрес не запрашивается снова.
...
npm run backup -- 2026-08-28   # докачать только их в backup/latest/
...
```bash
npm run backup -- --fresh # единственный сетевой шаг; атомарно заменяет backup/latest/
npm run icons            # при необходимости обновить PNG-иконки
npm run build            # backup/latest/ + site/ → crawled/ → docs/ → Pagefind → verify
npm run print-structure  # обновить список страниц для README
```

`backup/latest/` содержит только актуальный завершённый снимок; дата хранится в его манифесте.
`npm run generate` доступен отдельно для пользователей, которым нужен только удобный для чтения
Markdown-каталог `crawled/`.
...
|---|---|---|
| `tools/crawl.ts` | создаёт/дополняет `backup/latest/` с SHA-256, редиректами и причинами пропуска | да |
...
├── backup/latest/             # актуальный исходный снимок и SHA-256 manifest
...
ссылается на первоисточник. Машинные копии страниц хранятся в `backup/latest/`; дата снятия и
контрольные суммы записаны в манифесте.
