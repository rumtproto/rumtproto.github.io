...
section: api
description: "Боты предоставляют набор команд, которыми пользователи могут пользоваться в личной переписке или в чате."
layout: layout.njk
---

# Команды ботов

[Боты](https://core.telegram.org/bots) предлагают набор [команд](https://core.telegram.org/bots/features#commands), которые пользователи могут вызывать в личной переписке или в чате.

Упрощённое описание в терминах HTTP-интерфейса Bot API см. [здесь »](https://core.telegram.org/bots/features#commands).
...
Конструкторы [botInfo](/constructor/botInfo/) внутри [userFull](/constructor/userFull/), [chatFull](/constructor/chatFull/), [channelFull](/constructor/channelFull/) содержат список команд, а для групп — также ID и описание каждого бота.

В графических клиентах, когда пользователь начинает сообщение с символа `/`, следует показывать список команд, которые поддерживают все боты, присутствующие в текущем чате; то же самое следует делать в личной переписке с самим ботом.

[@term:bot_info_version] Если список команд бота изменился, изменится и `bot_info_version` (версия сведений о боте) в конструкторе [user](/constructor/user/), приходящем в обновлениях; это признак того, что клиенту следует заново запросить полные сведения о боте методом [users.getFullUser](/method/users.getFullUser/).
...
Список команд может изменить владелец бота через [@botfather](https://t.me/botfather), но и сам бот может менять свой список команд вызовом [bots.setBotCommands](/method/bots.setBotCommands/).
