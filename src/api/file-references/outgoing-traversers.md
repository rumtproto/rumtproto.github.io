---
title: "Исходящие обходчики ссылок на файлы"
original: "https://core.telegram.org/api/file-references/outgoing-traversers"
section: api
description: "Полный список исходящих обходчиков, содержащихся в текущем файле карты ссылок на файлы » — file_reference здесь означает ссылку на файл, короткую метку, которую сервер выдаёт…"
layout: layout.njk
---

# Исходящие обходчики ссылок на файлы

[@term:file_reference] Полный список исходящих обходчиков из текущего [файла карты ссылок на файлы »](/api/file-references/#map-file) — `file_reference` здесь означает ссылку на файл: короткую метку, которую сервер выдаёт вместе с файлом и которую клиент обязан предъявлять при каждом обращении к этому файлу.

Подробнее о том, как работают исходящие обходчики, см. [здесь »](/api/file-references/#outgoing-traversers).
