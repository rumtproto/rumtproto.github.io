...
section: api
description: "Ниже приведена актуальная TL-схема базы данных ссылок на файлы (file_reference — короткий идентификатор, подтверждающий право клиента на скачивание конкретного файла), которая…"
layout: layout.njk
---

# Схема БД

[@term:file_reference] Это актуальная TL-схема [базы данных ссылок на файлы](/api/file-references/) (`file_reference` — короткая метка, которая привязывает файл к источнику, откуда он получен, и подтверждает, что ссылка не устарела); её содержит текущий [файл карты ссылок на файлы »](/api/file-references/#map-file):
