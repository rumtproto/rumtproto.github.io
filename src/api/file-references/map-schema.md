---
title: "Схема файла карты ссылок на файлы"
original: "https://core.telegram.org/api/file-references/map-schema"
section: api
description: "Это TL-схема файла карты ссылок на файлы » — базы, в которой хранятся file_reference (короткие двоичные ссылки, подтверждающие право клиента скачать конкретный файл)."
layout: layout.njk
---

# Схема файла карты ссылок на файлы

[@term:file_reference] Это TL-схема [файла карты ссылок на файлы »](/api/file-references/) — машиночитаемого описания того, где в схеме встречаются `file_reference` (короткие метки с ограниченным сроком жизни, по которым клиент обращается к медиафайлу) и как их собирать и обновлять.

[Скачать TL-схему файла карты »](https://raw.githubusercontent.com/danog/MadelineProto/refs/heads/v8/src/TL_file_ref_map_schema.tl)

[Скачать JSON-версию TL-схемы файла карты »](https://raw.githubusercontent.com/danog/MadelineProto/refs/heads/v8/src/TL_file_ref_map_schema.json)

Подробнее о базе данных ссылок на файлы см. [здесь »](/api/file-references/).
...
// Points to a constructor or method
