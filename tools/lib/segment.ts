...
//
//   tools/harvest.ts  reads mirror + current Russian pages and pairs their
//                      segments positionally to seed the store;
//   tools/apply.ts    reads mirror pages, looks every segment up in the store
...
export function isTranslatable(text) {
  ...
  // Must contain a letter run of 2+ that is not glued inside inline code or
  // markup. Strip Markdown link destinations too: upstream 2.0 emits linked raw
  // <img> tags, whose visible alt/title belongs to the media translation pass;
  // the surrounding URL is structure, not a prose segment.
  const bare = t
    .replace(/`[^`]*`/g, ' ')
    .replace(/<[^>]+>/g, ' ')
    .replace(/\]\([^)]+\)/g, ']');
