...
import { htmlBlocks, isTranslatable, rewrite, segments } from './lib/segment.js';
import { isSchemaName } from './lib/schema-names.js';
import { isNeverTranslated } from './lib/never-translate.js';
import { russianFrontMatter } from './lib/frontmatter.js';
import { translateHtmlText, translateMedia } from './lib/media.js';
...
const mirrorPages = globSync('**/*.md', { cwd: MIRROR });
const mirrorRoutes = new Set(mirrorPages.map(routeFor));
...
// Generated indexes (notably /blog/) repeat page titles and derived description
// snippets inside raw HTML cards. Reuse the translations already derived for
// those pages instead of storing a second copy of every title and excerpt.
const generatedMeta = {};
for (const rel of mirrorPages) {
  ...
  if (doc.data.title && meta.title) generatedMeta[doc.data.title] = meta.title;
  if (doc.data.description && meta.description) generatedMeta[doc.data.description] = meta.description;
}
...
for (const rel of globSync('**/*.md', { cwd: MIRROR }).sort()) {
  ...
  const withHtml = translateHtmlText(
    body,
    htmlBlocks(body),
    (text) => local[text] ?? tm[text] ?? generatedMeta[text],
