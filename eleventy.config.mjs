...
import mirrorConfig from './mirror/eleventy.config.mjs';
...
function remapPaths(value) {
  if (typeof value === 'string') return value.replace(/^src(?=\/|$)/, 'build');
...
export default function russianConfig(eleventyConfig) {
  ...
  // The mirror compiles its stylesheet as a template so it can be minified; the
  // same treatment is given to the Russian-only script, so both additive assets
  // ship through the same pipeline as the mirror's own. `js.11tydata.json`
  // keeps it out of collections, exactly as the mirror does for its CSS.
  eleventyConfig.addTemplateFormats('js');
  eleventyConfig.addExtension('js', {
    outputFileExtension: 'js',
    compile: (inputContent) => async () => {
      const { code } = await minifyJs(inputContent);
      return code;
    },
  });
